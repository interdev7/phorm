...
class MetadataExtractor {
  static const columnChecker = TypeChecker.fromUrl('package:sqflow_platform_interface/src/annotations.dart#Column');
  static const idChecker = TypeChecker.fromUrl('package:sqflow_platform_interface/src/annotations.dart#ID');
  static const foreignKeyChecker = TypeChecker.fromUrl('package:sqflow_platform_interface/src/annotations.dart#ForeignKey');
  static const belongsToChecker = TypeChecker.fromUrl('package:sqflow_platform_interface/src/annotations.dart#BelongsTo');
  static const manyToManyChecker = TypeChecker.fromUrl('package:sqflow_platform_interface/src/annotations.dart#ManyToMany');
  static const valueConverterChecker = TypeChecker.fromUrl('package:sqflow_platform_interface/src/value_converter.dart#ValueConverter');
    ...
    try {
      ...
      if (element is ClassElement) {
        final idField = element.fields.firstWhere(
          (f) => f.metadata.any((m) => m.element?.enclosingElement3?.name == 'ID'),
        );
  ...
  static String resolveSqlType(FieldElement field) {
    final annotation = columnChecker.firstAnnotationOf(field) ?? idChecker.firstAnnotationOf(field);
  ...
  static ConverterInfo? getConverterInfo(FieldElement field) {
    ...
    final valueConverterType = [converterType, ...converterType.allSupertypes].firstWhere(
      ...
      orElse: () => throw Exception('Converter ${converterType.element.name} must inherit from ValueConverter'),
    );
