# SQFlow — Reactive Streams & Real-time Watchers

SQFlow features a robust, state-of-the-art reactive architecture that enables your UI to update in real time as the database changes. You can watch single records or entire datasets using simple stream-based APIs.
...
`SqflowCore<T>` provides two primary methods to watch data changes:
...
SQFlow solves this beautifully through **Dependency Tracking**:

### Automatic Include-based Dependencies

When using `include` to load relations, SQFlow **automatically detects** all related tables and registers them as dependencies for the stream.

```dart
// SQFlow automatically registers 'posts' as a dependency because of the include block.
...
When you execute multiple operations in a transaction, the UI shouldn't undergo multiple rapid redraws (flashing/flickering). SQFlow buffers all updates and notifies your streams only after the transaction is fully committed:
